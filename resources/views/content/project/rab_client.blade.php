                    ...
                    @foreach($output as $key => $value)
                        <table>
                            ...
                            <tr>
                                <td>Download file</td>
                                <td>:</td>
                                <td><a href="{{ url('project/rab/'.$id.'/download') }}">Download file</a> </td>
                            </tr>
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach($value['detail_rab'] as $item)
                                <tr>
                                    <td>{{ $no+=1 }}</td>
                                    <td>{{ $item->master_job }}
                                    <td>{{ $item->sub_job_name }}</td>
                                    <td>{{ $item->unit }}</td>
                                    <td>{{ $item->volume }}</td>
                                    <td>{{ $item->start_date }}</td>
                                    <td>{{ $item->estimate_end_date }}</td>
                                    <td>{{ $item->end_date }}</td>
                                    <td>{{ $item->late_reason }}</td>
                                    <td>{{ $item->last_progress }} %</td>
                                    <td>
                                        @if($item->status_sub == "0")
                                            <span class="label label-default">On Progress</span>
                                        @elseif($item->status_sub == "1")
                                            <span class="label label-success">Finish</span>
